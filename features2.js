...
// ─────────────────────────────────────────────────────────────────────────────
const BROKERS = [
  { id: 'interactiveBrokers', name: 'Interactive Brokers', icon: '🏦', hint: 'Activity Statement CSV' },
  { id: 'degiro',             name: 'DEGIRO',              icon: '🇩🇪', hint: 'Transaktionen.csv' },
  { id: 'tradeRepublic',      name: 'Trade Republic',      icon: '📱', hint: 'Transaktionshistorie CSV' },
  { id: 'coinbase',           name: 'Coinbase',            icon: '🪙', hint: 'Standard CSV-Export' },
  { id: 'binance',            name: 'Binance',             icon: '🟡', hint: 'Trade History CSV' },
  { id: 'kraken',             name: 'Kraken',              icon: '🐙', hint: 'Ledger CSV' },
  { id: 'generic',            name: 'Andere / Manuell',    icon: '📄', hint: 'MAERMIN Standard CSV / JSON' },
];

function BrokerImportWizard({ theme, t, addToast, onImport }) {
  const [step, setStep]             = useState(0); // 0=select, 1=upload, 2=preview, 3=done
  ...
  useEffect(() => {
    if (!rawData || !selectedBroker) return;
    try {
      const engine = window.ImportExportEngine;
      if (!engine) { addToast && addToast('ImportExportEngine nicht geladen', 'error'); return; }
      const result = engine.importData(rawData, 'csv', { broker: selectedBroker });
      if (result && result.transactions) {
        setParsed(result.transactions);
      } else {
        // Fallback: generic CSV parse
        const rows = engine.parseCSV ? engine.parseCSV(rawData) : [];
        setParsed(Array.isArray(rows) ? rows : []);
      }
    } catch(e) {
  ...
  const btn = (label, onClick, primary=false, disabled=false) =>
    React.createElement('button', {
      onClick, disabled,
      style: {
        padding: '0.625rem 1.25rem', border: 'none', borderRadius: '8px', cursor: disabled ? 'not-allowed' : 'pointer',
  ...
  const steps = ['Broker wählen', 'Datei laden', 'Vorschau', 'Fertig'];

  return React.createElement('div', { style: { padding: '1.5rem' } },
    React.createElement('h2', { style: { color: theme.text, fontSize: '1.5rem', fontWeight: '700', marginBottom: '1.25rem' } }, '📥 ' + (t.brokerImport || 'Broker-Import')),
    ...
    React.createElement('div', { style: { display: 'flex', gap: '0', marginBottom: '2rem', borderRadius: '10px', overflow: 'hidden', border: `1px solid ${theme.cardBorder}` } },
      steps.map((s, i) =>
        React.createElement('div', {
          key: i,
          style: {
            flex: 1, padding: '0.625rem', textAlign: 'center', fontSize: '0.8rem', fontWeight: i === step ? '700' : '400',
      ...
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: '0.75rem' } },
        BROKERS.map(b =>
          React.createElement('div', {
            ...
            React.createElement('span', { style: { fontSize: '1.5rem' } }, b.icon),
            React.createElement('div', null,
              React.createElement('div', { style: { color: theme.text, fontWeight: '600', fontSize: '0.9rem' } }, b.name),
              React.createElement('div', { style: { color: theme.textSecondary, fontSize: '0.75rem' } }, b.hint)
            )
          )
        )
      )
    ...
    step === 1 && React.createElement('div', null,
      React.createElement('div', {
        ...
        style: {
          border: `2px dashed ${theme.cardBorder}`, borderRadius: '12px', padding: '3rem',
          textAlign: 'center', cursor: 'pointer', marginBottom: '1rem', transition: 'all 0.15s'
        }
      },
        ...
        React.createElement('div', { style: { color: theme.textSecondary, fontSize: '0.875rem' } }, `Oder klicken zum Auswählen · ${BROKERS.find(b=>b.id===selectedBroker)?.name}`),
      ...
      React.createElement('p', { style: { color: theme.textSecondary, fontSize: '0.8rem', lineHeight: '1.6' } },
        'Hinweis: Alle Daten bleiben lokal in deinem Browser. Es wird nichts hochgeladen.'
      ),
      React.createElement('div', { style: { display: 'flex', gap: '0.5rem', marginTop: '1rem' } },
        btn('← Zurück', () => setStep(0)),
        btn('Oder Text einfügen', () => { setRawData(' '); setStep(2); })
      )
    ),
    ...
    step === 2 && React.createElement('div', null,
      ...
      parsed.length === 0 && React.createElement('div', null,
        React.createElement('p', { style: { color: theme.warning, marginBottom: '1rem' } }, '⚠️ Keine Transaktionen erkannt. Füge die CSV-Daten hier ein:'),
        React.createElement('textarea', {
          value: rawData === ' ' ? '' : rawData,
          onChange: e => setRawData(e.target.value),
          placeholder: 'CSV oder JSON Inhalt einfügen...',
      ...
      parsed.length > 0 && React.createElement('div', null,
        React.createElement('div', { style: { color: '#22c55e', fontWeight: '600', marginBottom: '0.75rem', fontSize: '0.9rem' } },
          `✓ ${parsed.length} Transaktionen erkannt`
        ),
