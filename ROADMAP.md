...
|---|---|---|---|
| **1 Real assets & property** | `real-assets.js`, `features5.js` (`NetWorthView`), `metrics.js` | ✅ shipped — property/vehicles/valuables with valuation history, acquisition cost+fees, optional financing link, recurring/one-off cashflows · pure `currentValue/netValue/netYield/totalReturn/valueSeries/aggregate` (EUR via `MaerminUtils`) · folds into Net Worth, gross value into `computeNetWorth` (no double-count of financing) · key `maermin_real_assets`, in backup | `test/real-assets.test.js` (27) |
| **2 Interest-bearing cash / time deposits** | `interest-engine.js`, `features5.js`, `renderer.js` | ✅ shipped — day-accurate act/365 accrual (daily/monthly/annual compounding), new `time_deposit` type, maturity stop · idempotent catch-up books `type:'interest'` transactions for the tax report (per `accountId`+`periodEnd`) · ledger key `maermin_interest_ledger`, in backup | `test/interest-engine.test.js` (27) |
| **3 Tax advisor & crypto Freigrenze countdown** | `tax-advisor.js`, `renderer.js` (German Tax view) | ✅ shipped — per-lot §23 EStG tax-free date + days remaining, 1.000 EUR crypto Freigrenze buffer, Sparerpauschbetrag headroom (1.000/2.000 married), loss-harvesting with stock vs other pots separated · ranked critical/important/optimization · pure consumer (no key) | `test/tax-advisor.test.js` (23) |
| **4 Performance map (treemap)** | `performance-map.js`, `renderer.js` (Performance view) | ✅ shipped — dependency-free squarified treemap, area=weight, colour=performance (theme-remapping scale), period selector w/ total-return fallback, SVG, privacy-mode masking · derived (no key) | `test/performance-map.test.js` (17) |
| **5 Market-cap size buckets** | `market-cap.js`, `cf-worker/worker.js` (`?action=fundamentals` adds `marketCap`), `investment-views.js` (Strategy) | ✅ shipped — EUR-normalised Large/Mid/Small/Unknown buckets (weights sum 100%), 30-day cache + `prefetchCaps` over the Worker · derived cache `maermin_marketcap_cache` (not backed up) · `docs/WORKER.md` updated | `test/market-cap.test.js` (25) |
| **6 Yield-on-cost + DRIP** | `dividend-yoc.js`, `renderer.js` (Dividends view) | ✅ shipped — annualised dividend over FIFO cost basis + per-buy YoC trend, DRIP simulation (reinvest at the day's price vs cash) clearly a simulation — never books real transactions · derived (no key) | `test/dividend-yoc.test.js` (24) |
| **7 Exchange read-only sync (crypto)** | `exchange-sync.js`, `cf-worker/worker.js` (`?action=brokerproxy`), `renderer.js` (Import) | ✅ shipped — read-only import for Binance/Kraken/Coinbase/Bitpanda over the existing relay · keys ONLY in the encrypted vault (never in state/URL/backup) · `validateReadOnly` rejects write/trade/withdraw scopes · pure adapters + idempotent dedupe/mergeSync · key `maermin_exchange_sync` (connection metadata only, in backup) · `docs/WORKER.md` + allowlist add `api.bitpanda.com` | `test/exchange-sync.test.js` (29) |
| **8 Reusable CSV import presets** | `import-mapping.js`, `features2.js` (Broker Import wizard) | ✅ shipped — named column-mapping presets, `applyPreset` reproduces a saved mapping and degrades absent columns to null · save/load/delete in the mapping preview · key `maermin_import_presets`, in backup | `test/import-presets.test.js` (21) |
| **9 MCP read-only endpoint** | `cf-worker/worker.js` (`?action=mcp`), `share-snapshot.js` | ✅ shipped — MCP-compatible read-only view over the redacted share snapshot (percentage weights + bounded scores only; never amounts/quantities/symbols), re-validated through the allowlist on the way out · same opt-in, time-limited link (expired id → 404) · `docs/WORKER.md` updated | `test/mcp-endpoint.test.js` (18) |
