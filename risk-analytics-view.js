...
window.createRiskAnalyticsView = function(props) {
  ...
  if (allPositions.length === 0) {
    return React.createElement('div', { 
      ...
      }, 'Füge Assets im Portfolio Tab hinzu um umfassende Risk Analytics zu sehen. Du erhältst dann:'),
      React.createElement('div', { 
        style: { 
          textAlign: 'left',
        ...
        React.createElement('div', { style: { marginBottom: '0.75rem', opacity: 0.8 } }, '• Portfolio Risk Score'),
        React.createElement('div', { style: { marginBottom: '0.75rem', opacity: 0.8 } }, '• Value at Risk (VaR) Berechnungen'),
  ...
  const concentration = analyzeConcentration(allPositions);
  const var95 = calculateValueAtRisk(allPositions, 0.95, 1);
  const var99 = calculateValueAtRisk(allPositions, 0.99, 1);
  const withVolatility = calculateVolatilityMetrics(allPositions);
  const withLiquidity = calculateLiquidityScores(allPositions);
  const riskScore = calculatePortfolioRiskScore(concentration, var95, withVolatility, withLiquidity);
